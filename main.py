...
def extract_products_details(row):
    ...
    try:
        title_cell = row.find("th", class_="custom__table-heading__title")
        if title_cell:
        ...
        general_cells = row.find_all("th", class_="custom__table-heading__general")
...
def crawl_shl_catalog_listing(base_url, output_csv, delay=1):
    ...
    while True:
        url = f"{base_url}?start={start_value}&type=1&type=1"
...
def main():
    ...
    output_dir = "app"
    output_csv = os.path.join(output_dir, f"shl_catalog_listing_{timestamp}.csv")
